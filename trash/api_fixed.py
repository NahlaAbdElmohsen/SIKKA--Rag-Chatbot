...
def format_routes_as_context(origin: str, destination: str, routes: dict) -> str:
    """
    Convert routing results into Arabic text that is injected into the LLM prompt
    as structured context. The LLM only formats language — routing is done here.
    """
    lines = []

    if routes["direct"]:
        lines.append(f"=== خطوط مباشرة من {origin} إلى {destination} ===")
        for r in routes["direct"]:
            lt = f" ({r['line_type']})" if r["line_type"] else ""
            lines.append(
                f"- من محطة {r['station']} إلى {destination}{lt} "
                f"بسعر {r['price']} جنيه"
            )

    if routes["one_transfer"]:
        lines.append(f"\n=== خطوط غير مباشرة (تحويلة واحدة) من {origin} إلى {destination} ===")
        for r in routes["one_transfer"]:
            f1, f2 = r["first_leg"], r["second_leg"]
            lt2 = f" ({f2['line_type']})" if f2["line_type"] else ""
            lines.append(
                f"- {origin} ← محطة {f1['station']} ← {r['via']} "
                f"(بسعر {f1['price']} جنيه) "
                f"ثم من محطة {f2['station']} → {destination}{lt2} "
                f"(بسعر {f2['price']} جنيه) | "
...
- إذا كانت هناك معلومات مسار محددة في قسم "نتائج البحث عن المسار"، فاستخدمها أولاً وحصراً للإجابة عن الخطوط والأسعار.
- إذا وجدت خطاً مباشراً، اذكره أولاً مع سعره.
- إذا وجدت خطوطاً غير مباشرة (بتحويلة)، اذكرها بعد ذلك مع تفصيل كل رحلة وسعرها والإجمالي.
- إذا لم توجد أي خطوط في المعلومات، قل ذلك بوضوح ولا تخترع مسارات.
- إذا لم تكن المعلومات كافية، قل فقط: "عذرًا، لا أستطيع مساعدتك في هذا الاستفسار".
