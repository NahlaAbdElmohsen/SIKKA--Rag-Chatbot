...
CITY_VARIANTS = [{'الإسكندرية', 'اسكندرية', 'اسكندريه', 'الاسكندرية'},
                 ...
                 {"أسوان","اسوان"},
                 {"الأقصر","الاقصر"},
                 {"جنوب سيناء","جنوب سينا"},
                 {"شمال سيناء","شمال سينا"},
                 {"الغربية","الغربيه"},
                 {"القليوبية","القليوبيه"},
                 {"المنوفية","المنوفيه"}]
...
def resolve_to_gov(graph: dict, city: str) -> list[str]:
     """
    Maps a user's free-text input to a list of city keys in the graph.
 
    Handles three cases:
      1. Exact city match (e.g. "المنصورة" → ["المنصورة"])
      2. Governorate match — any destination whose dest_gov equals the query
         (e.g. "الدقهلية" → ["المنصورة", "طلخا", ...])
      3. No match → returns []
 
    This is the key function that lets users type a governorate
    instead of an exact city name and still get valid results.
    """
     # First try exact city match
     if city in graph:
         return [city]
     # If no exact city match, try governorate match
     matched = set()
     for _, legs in graph.items():
        for leg in legs:
            if leg.get("destination_gov") == city:
                matched.add(leg["destination"])
     return list(matched)
...
def get_all(graph:dict,origin:str,destination:str)->list:
    """
    Main entry point for route search.
 
    Resolves both origin and destination from the user's free-text input
    (handles exact city names OR governorate names), then returns all
    direct and indirect routes found across all matching city pairs.
    """
    org_cities = resolve_to_gov(graph, origin)
    dest_cities = resolve_to_gov(graph, destination)
    all_results = []
    for org in org_cities:
        for dest in dest_cities:
            all_results.extend(find_routes(graph, org, dest))
    return all_results
...
def hybrid_retrieve(
    ...
    if origin and destination:
        org_variants = get_all_variants(origin)
        dest_variants = get_all_variants(destination)
        ...
        try:
            ...
            results = pinecone_index.query(
                ...
                filter={
                    "$and": [
                        {"$or": [{"city": {"$in": org_variants}}, {"governorate": {"$in": org_variants}}]},
                        {"$or": [{"destination": {"$in": dest_variants}}, {"destination_gov": {"$in": dest_variants}}]},
                    ]
                },
            )
    ...
    all_routes = get_all(route_graph, origin, destination)
    if all_routes:
        print(f"[Retrieval] Layer 2: found {len(all_routes)} route(s).")
        # Combine and return the found routes
        return routes_to_documents(all_routes)
...
async def lifespan(app: FastAPI):
    ...
    prompt_template = """أنت مساعد ذكي متخصص في الاستعلام عن خطوط السفر بالحافلات وأسعار رحلات السفر.
     أجب عن سؤاله بناءً على المعلومات المتاحة لديك، مع اتباع قواعد الاستجابة التالية بدقة:
